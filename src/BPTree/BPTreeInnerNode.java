...
public class BPTreeInnerNode<T extends Comparable<T>> extends BPTreeNode<T>  implements Serializable
{
	...
	public BPTreeInnerNode(int n,String indexColumn) 
	{
		super(n,indexColumn);
	 ...
	 */
	public PushUp<T> insert(T key, TupleRef recordReference, BPTreeInnerNode<T> parent, int ptr)
	{
		...
		{
			index = 0;
			while (index < numberOfKeys && Functions.compareObjects(getKey(index), ((Tuple)key).get(getIndexColumn())) < 0)
				++index;
	...
	public BPTreeInnerNode<T> split(PushUp<T> pushup) 
	{
		...
		BPTreeInnerNode<T> newNode = new BPTreeInnerNode<T>(order,this.getIndexColumn());
	...
	public LinkedList<Tuple> smallerequalSearch(SQLTerm indexedTerm, SQLTerm[] otherSQLTerms, String[] strarrOperators) {
		return getFirstChild().smallerequalSearch(indexedTerm, otherSQLTerms, strarrOperators);
	...
	public LinkedList<Tuple> greaterequalSearch(SQLTerm indexedTerm, SQLTerm[] otherSQLTerms,String[] strarrOperators) {
		T tuple = (T) new Tuple(indexedTerm.get_strColumnName());
		((Tuple) tuple).put(indexedTerm.get_strColumnName(), indexedTerm.get_objValue());
		return children[findIndex(tuple)].greaterequalSearch(indexedTerm, otherSQLTerms, strarrOperators);
	}
	...
	public LinkedList<Tuple> equalSearch(SQLTerm indexedTerm, SQLTerm[] otherSQLTerms, String[] strarrOperators) {
		T tuple = (T) new Tuple(indexedTerm.get_strColumnName());
		((Tuple) tuple).put(indexedTerm.get_strColumnName(), indexedTerm.get_objValue());
		return children[findIndex(tuple)].equalSearch(indexedTerm, otherSQLTerms, strarrOperators);
	}

	@Override
	public LinkedList<Tuple> notEqualSearch(SQLTerm indexedTerm, SQLTerm[] otherSQLTerms, String[] strarrOperators) {
		return getFirstChild().notEqualSearch(indexedTerm, otherSQLTerms, strarrOperators);
	}

	@Override
	public TupleRef insertLocation(T tuple) {
		return children[findIndex((tuple))].insertLocation(tuple);
	}

//	@SuppressWarnings("unchecked")
//	@Override
//	public TupleRef searchToDelete(Hashtable<String, Object> deleteColumnValues) {
//		T tuple = (T) new Tuple(getIndexColumn());
//		((Tuple)tuple).put(getIndexColumn(), deleteColumnValues.get(getIndexColumn()));
//		return children[findIndex(tuple)].searchToDelete(deleteColumnValues);
//	}
	...
	public void updateTupleRef(T tuple, TupleRef tupleRef) {
		children[findIndex(tuple)].updateTupleRef(tuple, tupleRef);
	}

	@Override
	public void printLeafNodes() {
		getFirstChild().printLeafNodes();
	}
